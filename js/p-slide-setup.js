...
const navMonths = document.querySelectorAll("nav .month");
//검색창
...
//제목이 쓰여진 div의 배열 (674개))
const titleDivs = titles.map((title) => {
...
/* (num) 메인 페이지에서 클릭한 날짜의 인덱스 */
let drawingIndex = Number(localStorage.getItem("drawingIndex"));
/* (date) 현재 그림의 날짜, 초기값은 첫 날로 설정 */
let currentFullDate = new Date('2020-06-19'); 
/* (num) 현재 연, 월, 일 */
let y; let m; let d;

/* (date) 현재 월의 첫 날, 다음 월의 첫 날 */
let firstDayofMonth; let firstDayofNextMonth;
/* (num) 현재 월의 날 개수 */
let daysInMonth;
/* (num) 현재 월의 첫 날의 요일을 숫자로 표시 */
let firstDayName; 

/* (num) 날짜 바에서 날짜가 몇 번째 박스에서 시작되는지 */
let startBoxNum;
/* (node로 이루어진 array) 날짜 바에서 1일부터 마지막 날까지 채워지는 박스 */
let filledBoxes = [];

/* (boolean) 메뉴 온오프 */
let navOn = false;
/* (num) 메뉴에서 클릭한 연도, 초기값은 현재 날짜의 연도 */
let currentNavYear;

/* (txt) 검색창에 입력한 텍스트 */
let searchedText;
/* (node로 이루어진 array) 검색 목록에 뜬 제목들 */
let searchedTitleDivs = [];
/* (num) 사용자가 가리키고 있는 제목의 인덱스 */
let searchIndex;
/* (boolean) 검색 목록에서 선택을 했는지 여부 */
...
function getDateInfo(date) {
    y = date.getFullYear();
    m = date.getMonth(); //0이면 1월, 1이면 2월, ... , 11이면 12월
    d = date.getDate();
